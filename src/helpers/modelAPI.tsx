...
import { clickType } from "./Interface"; // 枚举值

const API_ENDPOINT = import.meta.env.VITE_API_ENDPOINT;
const ALL_MASK_API_ENDPOINT = import.meta.env.VITE_ALL_MASK_API_ENDPOINT;
const ERASE_API_ENDPOINT = import.meta.env.VITE_ERASE_API_ENDPOINT;
...
const queryModelReturnTensors = async ({
  ...
  console.log(ALL_MASK_API_ENDPOINT);
  console.log(ERASE_API_ENDPOINT);
  if (!API_ENDPOINT) return;
  if (!ALL_MASK_API_ENDPOINT) return;
  const segRequest =
    imgName && !shouldDownload
      ? fetch(`/gallery/${imgName}.txt`)
      : fetch(`${API_ENDPOINT}`, {
          method: "POST",
          body: blob,
        });
